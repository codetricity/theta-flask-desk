...
def last_image():
    ...
    return render_template(
        "index.html",
        image_url=last_file_url,
        title=name + " last image",
        active_nav="last_image",
    )
...
def full_image():
    ...
    return render_template(
        "image.html", image_url=image_url, title=name + " image in camera"
    )
...
def ten_images():
    ...
    payload = {
        "name": command_string,
        "parameters": {"fileType": "image", "entryCount": 10, "maxThumbSize": 0},
    }
    resp = requests.post(url, json=payload)
    ...
    return render_template(
        "thumbs.html", entries=entries, title="Files on camera"
    )
...
def thirty_images():
    ...
    payload = {
        "name": command_string,
        "parameters": {"fileType": "image", "entryCount": 30, "maxThumbSize": 0},
    }
    resp = requests.post(url, json=payload)
